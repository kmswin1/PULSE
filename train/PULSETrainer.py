...
class PULSETrainer(Trainer):
    ...
    def __init__(
        ...
        self.steps_per_epoch = train_dataset.__len__() // (args.per_device_train_batch_size * args.gradient_accumulation_steps * torch.cuda.device_count()) # global batch size
        self.total_epochs = args.num_train_epochs
        self.beta_scheduling_ratio = beta_scheduling_ratio
    ...
    def tokenize_row(features, processing_class, max_prompt_length, max_completion_length, add_special_tokens):
        ...
        return {
            ...
            "length_chosen": len(chosen_input_ids),
            "length_rejected": len(rejected_input_ids),
        }
    ...
    def compute_ref_log_probs(self, batch: dict[str, torch.LongTensor]) -> dict:
        ...
        return ref_model_output["chosen_logps"], ref_model_output["rejected_logps"], ref_model_output["chosen_loss"], ref_model_output["rejected_loss"], ref_model_output["chosen_logits"], ref_model_output["rejected_logits"]
    ...
    def dpo_loss(
        ...
        elif self.loss_type == "pulse":
            chosen_logratios = (chosen_logps - ref_chosen_logps)
            rejected_logratios = (rejected_logps - ref_rejected_logps)

            beta = self.beta * min(round(self.training_steps / (self.beta_scheduling_ratio*(self.steps_per_epoch * self.total_epochs)), 3), 1)
            self.training_steps += 1

            losses = (1 / (1 + torch.exp(beta * (chosen_logratios - rejected_logratios))) ** 2) * (1 - self.label_smoothing) + (1 / (1 + torch.exp(-beta * (chosen_logratios - rejected_logratios))) ** 2) * self.label_smoothing
    ...
    def get_batch_loss_metrics(
        ...
        if "ref_chosen_logps" in batch and "ref_rejected_logps" in batch:
            ...
            ref_chosen_logps, ref_rejected_logps, ref_chosen_loss, ref_rejected_loss, ref_chosen_logits, ref_rejected_logits = self.compute_ref_log_probs(batch)
